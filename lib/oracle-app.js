...
/** The handler that actually serves a ticker's signal (shared by x402 and MPP surfaces). */
export function priceHandlerFor(s) {
...
export function attachOracle(app, { facilitatorUrl }) {
  ...
  for (const s of WATCHLIST) {
    app.get(pricePath(s.ticker), priceHandlerFor(s));
  }
}
